...
import { Person, Edit, Save, Cancel, PhotoCamera, ArrowBack } from '@mui/icons-material';
...
export const CustomerProfile = ({ profile, refreshProfile }: CustomerProfileProps) => {
  ...
  const handleEdit = () => {
    setFormData({
      ...
      dateOfBirth: profile.customer?.dateOfBirth ? new Date(profile.customer.dateOfBirth).toISOString().split('T')[0] : '',
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        setSaving(true);
        const imageData = { ...formData, firstName: profile.firstName || '', lastName: profile.lastName || '', profileImage: file };
        await profileApi.updateCustomerProfile(imageData);
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await profileApi.updateCustomerProfile(formData);
      ...
      <Paper sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          <Box sx={{ position: 'relative' }}>
            ...
            >
              {!previewUrl && !profile.profileImageUrl && <Person />}
            </Avatar>
            <Button
              component="label"
              sx={{
            ...
            <>
              ...
              <Grid item xs={12} md={6}>
                <TextField
                  fullWidth
                  label="Phone"
                  value={isEditing ? formData.phone : (profile.customer.phone || '')}
              ...
              <Grid item xs={12}>
                <TextField
                  fullWidth
                  label="Address"
                  value={isEditing ? formData.address : (profile.customer.address || '')}
